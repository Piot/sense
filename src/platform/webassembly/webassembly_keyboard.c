...
{
    char* keynames[] = {"w", "s", "a", "d", "q", "e", "r", "f", "esc", "i", "k", "j", "l", "o", "u", "y", "h", "n"};
...
static EM_BOOL on_key(sense_webassembly_keys* self, const EmscriptenKeyboardEvent* keyEvent, bool state)
{
    if (keyEvent->repeat) {
        return EM_FALSE;
    }
...
static EM_BOOL on_key_down(int eventType, const EmscriptenKeyboardEvent* keyEvent, void* user_data)
{
    ...
    return on_key((sense_webassembly_keys*) user_data, keyEvent, true);
}

static EM_BOOL on_key_up(int eventType, const EmscriptenKeyboardEvent* keyEvent, void* user_data)
{
    (void) eventType;
    return on_key((sense_webassembly_keys*) user_data, keyEvent, false);
}
